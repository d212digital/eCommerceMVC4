    ...
    public class ProductsController : DashboardBaseController
    {
        ...
        public ActionResult Action(int? ID)
        {
            ...
            if (ID.HasValue)
            {
                ...
                model.DiscountDetails = currentLanguageRecord.ProductDiscountDetails;
            }
        ...
        public JsonResult Action(FormCollection formCollection)
        {
            ...
            try
            {
                ...
                if (model.ProductID > 0)
                {
                    ...
                    var newProductDiscountDetails = new List<ProductDiscountDetails>();

                    if(model.DiscountDetails != null && model.DiscountDetails.Count > 0)
                    {
                        newProductDiscountDetails.AddRange(model.DiscountDetails.Select(x => new ProductDiscountDetails()
                        {
                            ProductRecordID = currentLanguageRecord.ID,
                            Fullwidth = x.Fullwidth,
                            Centrefoldedwidth=x.Centrefoldedwidth,
                            Rolllength=x.Rolllength,
                            Priceperroll_1to5=x.Priceperroll_1to5,
                            Priceperroll_6to15=x.Priceperroll_6to15,
                            Priceperroll_16to23=x.Priceperroll_16to23,
                            Priceperroll_24to47=x.Priceperroll_24to47,
                            Priceperroll_48=x.Priceperroll_48
                    ...
                    if (isNewRecord)
                    {
                        ...
                        currentLanguageRecord.ProductDiscountDetails = newProductDiscountDetails;
                    }
                    else
                    {
                        var productSpecsUpdated = ProductsService.Instance.UpdateProductSpecifications(product.ID, newProductSpecification);
                        var productDiscountUpdated = ProductsService.Instance.UpdateProductDiscountDetails(product.ID, newProductDiscountDetails);
                ...
                else
                {
                    ...
                    {
                        currentLanguageRecord.ProductDiscountDetails = new List<ProductDiscountDetails>();
                        currentLanguageRecord.ProductDiscountDetails.AddRange(model.DiscountDetails.Select(x => new ProductDiscountDetails()
                        {
        ...
        private ProductActionViewModel GetProductActionViewModelFromForm(FormCollection formCollection)
        {
            var model = new ProductActionViewModel
            {
                ...
                DiscountDetails = new List<ProductDiscountDetails>()
            };
            ...
            foreach (string key in formCollection)
            {
                if (key.Contains("discountdetial"))
                {
                    var value = formCollection[key];
                    ...
                    if (!string.IsNullOrEmpty(value))
                    {
                        ...
                        var discountDetail = value.Split('~');

                        var fullWidth = discountDetail[0];
                        var centreFoldedWidth = discountDetail[1];
                        var rollLength = discountDetail[2];
                        var priceperroll_1to5 = discountDetail[3];
                        var priceperroll_6to15 = discountDetail[4];
                        var priceperroll_16to23 = discountDetail[5];
                        var priceperroll_24to47 = discountDetail[6];
                        var priceperroll_48 = discountDetail[7];

                        if (!string.IsNullOrEmpty(fullWidth) && !string.IsNullOrEmpty(centreFoldedWidth))
                        {
                            model.DiscountDetails.Add(new ProductDiscountDetails()
                            {
                                Fullwidth = fullWidth,
                                Centrefoldedwidth = centreFoldedWidth,
                                Rolllength = rollLength,
                                Priceperroll_1to5 = Convert.ToDecimal(priceperroll_1to5),
                                Priceperroll_6to15 = Convert.ToDecimal(priceperroll_6to15),
                                Priceperroll_16to23 = Convert.ToDecimal(priceperroll_16to23),
                                Priceperroll_24to47 = Convert.ToDecimal(priceperroll_24to47),
                                Priceperroll_48 = Convert.ToDecimal(priceperroll_48)
                            });
                        }
                    }
                }
